...
class WorkoutController extends Controller
{
    ...
    {
        return view('master.work-out');
    }

    public function workOutSave(Request $request)
    {
        ...
        $validator = Validator::make(
            $params,
            [
                'name' => 'required|unique:work_out,name',
            ]
        );
    ...
    public function workoutAjaxcall(Request $request)
    {
    ...
    public function getworkout(Request $request)
    {
        ...
        if ($update) {
            $data = Workout::where('id', $update)->first();
        }

        if ($delete) {
            $data = Workout::where('id', $delete)->delete();
        }
    ...
    public function workoutupdate(Request $request)
    {
        ...
        $messages = [
            'name.unique' => 'The workout name must be unique. 
        ...
        $validator = Validator::make(
            $params,
            [
                'name' => 'required|unique:work_out,name,' . $request->id,
            ],
            $messages
        );
        ...
        $data = Workout::where('id', $request->id)->update($data);
