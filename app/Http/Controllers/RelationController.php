...
class RelationController extends Controller
{
    ...
    public function relationSave(Request $request)
    {
        ...
        $validator = Validator::make(
            $params,
            [
                'name' => 'required|unique:relation_master,name',
        ...
        if ($request->hasFile('logo') && $request->file('logo')->isValid()) {
            ...
            $image->move(public_path('relation'), $imageName);

            $logoPath = 'relation/' . $imageName;
    ...
    public function relationAjaxcall(Request $request)
    {
    ...
    public function getrelation(Request $request)
    {
        ...
        if ($update) {
            $data = Relation::where('id', $update)->first();
        }

        if ($delete) {
            $data = Relation::where('id', $delete)->delete();
        }
    ...
    public function relationupdate(Request $request)
    {
        ...
        $rules = [
            'name' => 'required|unique:relation_master,name,' . $request->id . ',id',
        ];
        ...
        Relation::where('id', $request->id)->update($data);
